                     ...
                     unsigned id)
{
    ...
    if (descriptorType.count(binding)) throw std::runtime_error("Duplicate binding");
    return binding;
}
    ...
    : mPushConstantSize(0)
{
    ...
    {
        mDescriptorTypes[ReadBinding(compiler, mDescriptorTypes, resource.id)] = vk::DescriptorType::eStorageBuffer;
    }

    // read storage images
    for (auto& resource: resources.storage_images)
    {
        mDescriptorTypes[ReadBinding(compiler, mDescriptorTypes, resource.id)] = vk::DescriptorType::eStorageImage;
    }

    // read combined samplers
    for (auto& resource: resources.sampled_images)
    {
        mDescriptorTypes[ReadBinding(compiler, mDescriptorTypes, resource.id)] = vk::DescriptorType::eCombinedImageSampler;
    }

    // read uniforms
    for (auto& resource: resources.uniform_buffers)
    {
        mDescriptorTypes[ReadBinding(compiler, mDescriptorTypes, resource.id)] = vk::DescriptorType::eUniformBuffer;
    }
