 ...
 *      a. Alternatively, you can clone `https://github.com/eqrion/cbindgen` and use a tagged
 * release
 *   2. Run `rustup run nightly cbindgen toolkit/library/rust/ --lockfile Cargo.lock --crate
 * wgpu-remote -o dom/webgpu/ffi/wgpu_ffi_generated.h`
 */
 ...
 *
 * Only relevant for [`BindingType::SampledTexture`] bindings. See [`TextureFormat`] for more
 * information.
 */
...
typedef enum WGPUTextureFormat
{
  ...
  /**
   * Red channel only. 8 bit integer per channel. [-127, 127] converted to/from float [-1, 1] in
   * shader.
   */
  ...
  /**
   * Red and green channels. 8 bit integer per channel. [0, 255] converted to/from float [0, 1] in
   * shader.
   */
  WGPUTextureFormat_Rg8Unorm = 7,
  /**
   * Red and green channels. 8 bit integer per channel. [-127, 127] converted to/from float [-1, 1]
   * in shader.
   */
  ...
  /**
   * Red, green, blue, and alpha channels. 8 bit integer per channel. Srgb-color [0, 255] converted
   * to/from linear-color float [0, 1] in shader.
   */
  WGPUTextureFormat_Rgba8UnormSrgb = 18,
  /**
   * Red, green, blue, and alpha channels. 8 bit integer per channel. [-127, 127] converted to/from
   * float [-1, 1] in shader.
   */
  ...
  /**
   * Blue, green, red, and alpha channels. 8 bit integer per channel. Srgb-color [0, 255] converted
   * to/from linear-color float [0, 1] in shader.
   */
  WGPUTextureFormat_Bgra8UnormSrgb = 23,
  /**
   * Red, green, blue, and alpha channels. 10 bit integer for RGB channels, 2 bit integer for alpha
   * channel. [0, 1023] ([0, 3] for alpha) converted to/from float [0, 1] in shader.
   */
  WGPUTextureFormat_Rgb10a2Unorm = 24,
  /**
   * Red, green, and blue channels. 11 bit float with no sign bit for RG channels. 10 bit float with
   * no sign bti for blue channel. Float in shader.
   */
 ...
 * - DX12
 * - Metal (with MSL 2.0+ on macOS 10.13+)
 * - Vulkan 1.2+ (or VK_EXT_descriptor_indexing)'s shaderSampledImageArrayNonUniformIndexing
 * feature)
 *
 * This is a native only feature.
 */
...
#define WGPUColor_TRANSPARENT \
  (WGPUColor) { .r = 0.0, .g = 0.0, .b = 0.0, .a = 0.0 }
#define WGPUColor_BLACK \
  (WGPUColor) { .r = 0.0, .g = 0.0, .b = 0.0, .a = 1.0 }
#define WGPUColor_WHITE \
  (WGPUColor) { .r = 1.0, .g = 1.0, .b = 1.0, .a = 1.0 }
#define WGPUColor_RED \
  (WGPUColor) { .r = 1.0, .g = 0.0, .b = 0.0, .a = 1.0 }
#define WGPUColor_GREEN \
  (WGPUColor) { .r = 0.0, .g = 1.0, .b = 0.0, .a = 1.0 }
#define WGPUColor_BLUE \
  (WGPUColor) { .r = 0.0, .g = 0.0, .b = 1.0, .a = 1.0 }
...
{
  /**
   * Operation to perform to the output attachment at the start of a renderpass. This must be clear
   * if it is the first renderpass rendering to a swap chain image.
   */
  WGPULoadOp load_op;
  /**
   * Operation to perform to the output attachment at the end of a renderpass.
...
typedef struct WGPURenderPassColorAttachmentDescriptorBase_TextureViewId
{
   ...
   * attachment has 1 sample (does not have MSAA). This is not mandatory for rendering with
   * multisampling, you can choose to resolve later or manually.
   */
...
typedef WGPURenderPassColorAttachmentDescriptorBase_TextureViewId
...
{
  /**
   * Texture attachment to render to. Must contain [`TextureUsage::OUTPUT_ATTACHMENT`] and be a
   * valid texture type for a depth/stencil attachment.
   */
  WGPUTextureViewId attachment;
  /**
   * Depth channel.
...
{
  /**
   * Offset into the buffer that is the start of the texture. Must be a multiple of texture block
   ...
   *
   * Must be a multiple of 256 for [`CommandEncoder::copy_buffer_to_texture`] and
   ...
   * May be larger than `copy_size.y`.
   *
 ...
 * [`CommandEncoder::copy_buffer_to_texture`], or [`Queue::write_buffer`] operation.
 */
...
typedef struct WGPUBufferDescriptor
{
  ...
  /**
   * Allows a buffer to be mapped immediately after they are made. It does not have to be
   * [`BufferUsage::MAP_READ`] or
...
{
  /**
   * Debug label for the command encoder. This will show up in graphics debuggers for easy
   * identification.
   */
  WGPULabel label;
...
typedef struct WGPUDepthStencilStateDescriptor
{
  ...
  /**
   * Stencil values are AND'd with this mask when reading and writing from the stencil buffer. Only
   * low 8 bits are used.
   */
  uint32_t stencil_read_mask;
  /**
   * Stencil values are AND'd with this mask when writing to the stencil buffer. Only low 8 bits are
   * used.
   */
 ...
 *
 * Arrays of these can be made with the [`vertex_attr_array`] macro. Vertex attributes are assumed
...
typedef struct WGPUTextureDescriptor
{
  ...
  WGPULabel label;
  /**
   * Size of the texture. For a regular 1D/2D texture, the unused sizes will be 1. For 2DArray
   * textures, Z is the number of 2D textures in that array.
   */
  WGPUExtent3d size;
  /**
...
{
  /**
   * Debug label of the render bundle encoder. This will show up in graphics debuggers for easy
   * identification.
   */
  WGPULabel label;
...
{
  /**
   * Debug label of the texture view. This will show up in graphics debuggers for easy
   * identification.
   */
  WGPULabel label;
  /**
   * Format of the texture view. At this time, it must be the same as the underlying format of the
   * texture.
   */
  WGPUTextureFormat format;
  /**
   * The dimension of the texture view. For 1D textures, this must be `1D`. For 2D textures it must
   * be one of `D2`, `D2Array`, `Cube`, and `CubeArray`. For 3D textures it must be `3D`
   */
  ...
  /**
   * Mip level count. Must be at least one. base_mip_level + level_count must be less or equal to
   * underlying texture mip count.
   */
  ...
  /**
   * Layer count. Must be at least one. base_array_layer + array_layer_count must be less or equal
   * to the underlying array count.
   */
...
WGPUBindGroupLayoutId wgpu_device_create_bind_group_layout(
    WGPUDeviceId device_id,
...
WGPUComputePipelineId wgpu_device_create_compute_pipeline(
    WGPUDeviceId device_id,
...
WGPURenderBundleEncoderId wgpu_device_create_render_bundle_encoder(
    WGPUDeviceId device_id,
