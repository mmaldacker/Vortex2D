...
vec2 bilerp(sampler2D u_texture, vec2 xy)
{
    ...
    vec2 t11 = texture2D(u_texture, st.xy).xy;
    vec2 t21 = texture2D(u_texture, st.xw).xy;
    vec2 t12 = texture2D(u_texture, st.zy).xy;
    vec2 t22 = texture2D(u_texture, st.zw).xy;
...
void main(void) 
{
    ...
    gl_FragColor = vec4(bilerp(u_texture, stepBackCoords), 0.0, 0.0);    
}