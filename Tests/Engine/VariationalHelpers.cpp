...
void SetVelocity(Vortex::Renderer::Device& device,
                 ...
                 FluidSim& sim)
{
  Vortex::Renderer::Texture input(device,
                                  size.x,
                                  size.y,
                                  Vortex::Renderer::Format::R32G32Sfloat,
                                  Vortex::Renderer::MemoryUsage::Cpu);
  ...
  device.Execute([&](Vortex::Renderer::CommandEncoder& command)
                 { velocity.CopyFrom(command, input); });
}
                 ...
                 float scale)
{
  Vortex::Renderer::Texture input(device,
                                  size.x,
                                  size.y,
                                  Vortex::Renderer::Format::R32Sfloat,
                                  Vortex::Renderer::MemoryUsage::Cpu);
  ...
  input.CopyFrom(phi);
  device.Execute([&](Vortex::Renderer::CommandEncoder& command)
                 { solidPhi.CopyFrom(command, input); });
}
                  ...
                  float scale)
{
  ...
  input.CopyFrom(phi);
  device.Execute([&](Vortex::Renderer::CommandEncoder& command)
                 { liquidPhi.CopyFrom(command, input); });
}
                         ...
                         FluidSim& sim)
{
  ...
  using Vortex::Renderer::CopyFrom;
...
void PrintWeights(const glm::ivec2& size, FluidSim& sim)
{
  for (int j = 1; j < size.y - 1; j++)
  {
    ...
    {
      int index = i + size.x * j;
      std::cout << "(" << sim.matrix(index + 1, index) << "," << sim.matrix(index - 1, index) << ","
                << sim.matrix(index, index + size.x) << "," << sim.matrix(index, index - size.x)
                << ")";
    }
    std::cout << std::endl;
  }
...
{
  Vortex::Renderer::Texture output(device,
                                   size.x,
                                   size.y,
                                   Vortex::Renderer::Format::R32G32Sfloat,
                                   Vortex::Renderer::MemoryUsage::Cpu);
  device.Execute([&](Vortex::Renderer::CommandEncoder& command)
                 { output.CopyFrom(command, velocity); });
                   ...
                   float error)
{
  ...
  Vortex::Renderer::CopyTo(bufferPressure, bufferPressureData);
  ...
  for (int i = 0; i < size.x; i++)
  {
    for (int j = 0; j < size.y; j++)
    {
      ...
      EXPECT_NEAR(value, bufferPressureData[index], error)
          << "Mismatch at " << i << ", " << j << "\n";
    }
  }
}
